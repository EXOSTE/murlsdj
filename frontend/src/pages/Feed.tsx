...
export default function Feed({ popular = false }: FeedProps) {
  ...
  const meta = useMediaMeta(activeItem);
  ...
  const loadPage = useCallback(async (p: number) => {
    ...
    try {
      const res = popular
        ? await getPopularMedia(p)
        : await getPublicMedia(p, undefined, debouncedSearch || undefined);
  ...
  return (
    <div className="relative w-screen h-dvh bg-black overflow-hidden">
      ...
      {!isKiosk && (
        <header className="absolute top-0 left-0 right-0 z-20 hidden sm:flex items-center justify-between px-5 py-4 bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
          ...
          <nav className="flex items-center gap-4 pointer-events-auto">
            ...
            >
              <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden="true"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
            </button>
            <button onClick={enterKiosk} aria-label="Activer le mode kiosque" className="text-white/60 hover:text-white transition-colors" title="Mode kiosque">
              <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden="true"><path d="M5 5h5V3H3v7h2zm5 14H5v-5H3v7h7zm11-5h-2v5h-5v2h7zm-2-9h2V3h-7v2h5z" /></svg>
            </button>
          </nav>
        </header>
      )}
      ...
      {!isKiosk && showSearch && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 w-full max-w-sm px-4 pointer-events-auto">
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/50" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <input
              ...
              className="w-full bg-black/60 backdrop-blur-md border border-white/20 rounded-full pl-10 pr-4 py-2.5 text-sm text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
            />
          </div>
        </div>
      )}
      ...
      {!isKiosk && (
        <header className="absolute top-0 left-0 right-0 z-20 flex sm:hidden items-center justify-between px-4 py-3 bg-gradient-to-b from-black/50 to-transparent pointer-events-none">
          ...
          <button onClick={enterKiosk} aria-label="Activer le mode kiosque" className="text-white/60 hover:text-white transition-colors pointer-events-auto" title="Mode kiosque">
            <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current" aria-hidden="true"><path d="M5 5h5V3H3v7h2zm5 14H5v-5H3v7h7zm11-5h-2v5h-5v2h7zm-2-9h2V3h-7v2h5z" /></svg>
          </button>
        </header>
      )}
      ...
      {/* Bottom nav mobile */}
      {!isKiosk && (
        <nav aria-label="Navigation principale" className="absolute bottom-0 left-0 right-0 z-20 flex sm:hidden items-center justify-around px-2 pb-safe bg-gradient-to-t from-black/70 to-transparent pointer-events-none">
          <Link to="/" aria-label="Souvenirs récents" aria-current={!popular ? "page" : undefined} className={`flex flex-col items-center gap-1 py-3 px-4 pointer-events-auto transition-colors ${!popular ? "text-white" : "text-white/50"}`}>
          ...
          <Link to="/populaire" aria-label="Souvenirs populaires" aria-current={popular ? "page" : undefined} className={`flex flex-col items-center gap-1 py-3 px-4 pointer-events-auto transition-colors ${popular ? "text-white" : "text-white/50"}`}>
            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden="true">
          ...
          <Link to="/contribuer" aria-label="Publier un souvenir" className="flex flex-col items-center gap-1 py-2 px-4 pointer-events-auto">
          ...
          <Link to="/galerie" aria-label="Galerie de souvenirs" className="flex flex-col items-center gap-1 py-3 px-4 pointer-events-auto text-white/50 hover:text-white transition-colors">
            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden="true">
          ...
          <Link to="/histoire" aria-label="Notre histoire — 30 ans du Silence des Justes" className="flex flex-col items-center gap-1 py-3 px-4 pointer-events-auto text-white/50 hover:text-white transition-colors">
            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden="true">
