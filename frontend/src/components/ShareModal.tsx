          ...
          <div className="grid grid-cols-2 gap-2">
            {options.map((opt) => (
              <a
                ...
                onClick={onClose}
                className={`${opt.color} flex items-center gap-2 px-3 py-3 rounded-xl transition-colors`}
              >
                <span className="shrink-0">{opt.icon}</span>
                <span className={`text-xs font-semibold truncate ${opt.textColor ?? "text-white"}`}>{opt.label}</span>
              </a>
            ))}
            ...
            <button
              onClick={handleCopy}
              className="bg-gradient-to-br from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] flex items-center gap-2 px-3 py-3 rounded-xl transition-opacity hover:opacity-90"
            >
              ...
              <span className="text-xs font-semibold text-white truncate">{copied ? "Copié !" : "Instagram"}</span>
            </button>

            {/* Autres applications */}
            <button
              onClick={handleNativeShare}
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center gap-2 px-3 py-3 rounded-xl transition-colors"
            >
              ...
              <span className="text-xs font-semibold truncate">Autres</span>
            </button>
          </div>
        </motion.div>
      </motion.div>
